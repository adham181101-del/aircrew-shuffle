...
async function applyShiftSwapFunction() {
  console.log('Applying execute_shift_swap function to Supabase...');
...
BEGIN
  ...
  IF counter_offer_date IS NOT NULL THEN
    ...
    SET status = 'accepted', accepter_id = accepter_id, accepted_at = NOW()
...
COMMENT ON FUNCTION public.execute_shift_swap IS 'Executes shift swaps with elevated privileges to bypass RLS restrictions. Used for counter-offers and direct swaps.';
  ...
  try {
    ...
    console.log('✅ execute_shift_swap function applied successfully!');
    return true;
  } catch (err) {
...
applyShiftSwapFunction().then(success => {
  if (success) {
    console.log('🎉 Database function is now ready for use!');
  } else {
    console.log('❌ Failed to apply database function');
  }
